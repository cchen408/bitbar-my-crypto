...
async function myCrypto() {
    ...
    console.log('---');
    console.log(`total: ${_.round(totalValue, 2)} | color='#000' font=${font}`);
    console.log('---');
    console.log(`${'sym'.padEnd(5)}\t${'price'.padEnd(8)}\t${'value'.padEnd(10)}\t${'cost'.padEnd(4)}\tport% | color='#000' font=${font}`);
    console.log('---');
    _.each(holdings, function (data) {
        data.cost = data.cost || 0;
        console.log(`${data.symbol.padEnd(5)}\t$${_.round(data.price, 5).toString().padEnd(8)}\t$${_.round(data.value, 2).toString().padEnd(10)}\t$${_.round(data.cost, 2).toString().padEnd(4)}\t${_.round(data.percentage, 2)}% | color='#000' font=${font} image=${icons[_.toLower(data.symbol)]}`);
    });
}
